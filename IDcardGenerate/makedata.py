...
def makedataset(avatar, im):
    ...
    im = changeBackground(avatar, im, (avalong, avaheight),  # 抠图
                          (centerrow, centercow))
    # im = paste(avatar, im, (avalong, avaheight),  # 贴图
    #            (centerrow, centercow))
...
all_avatar_path = glob.glob('./augmented_data/imagesup/*')
all_avatar_path_down = glob.glob('./augmented_data/imagesdown/*')
for i in range(25):
    ...
    img.save("./last_noise/imagesup/"+"noise"+str(i)+".png")

    ava_path = random.sample(all_avatar_path_down, 1)
    ava_path = ''.join(ava_path)
    avatar = PImage.open(ava_path)
    img = makedataset(avatar, im)
    img.save("./last_noise/imagesdown/"+"noise"+str(i)+".png")
    print("finish:%d" % i)
